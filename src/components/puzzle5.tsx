...
import Solution from '../images/sol5.webp';
...
import { decrement, increment, five } from '../features/puzzNumber/puzzNumberSlice';
...
const Puzzle5: React.FC = () => {
  ...
  const Increment = (ev) => {
    ...
    const clickedCellText: string = document.getElementById(
      `${ev.id}`
    ).innerHTML;
  ...
  const DidYaWinSon: () => void = () => {
    ...
    if (
      ...
      const schlepper: HTMLElement =
        document.querySelector('.winning-container');
      schlepper.style.display = 'grid';
    }
  };

  const puzzleFiveArray: string[] = [
    'a1',
    'a2',
    'a3',
    'a4',
  ...
  const GetLocalStorage: () => void = () => {
    for (let i: number = 0; i < puzzleFiveArray.length; i++) {
      if (localStorage.getItem(`5${puzzleFiveArray[i]}`) !== (null || '0')) {
        let loadedCell: HTMLElement = document.getElementById(
          `${puzzleFiveArray[i]}`
        );
  ...
  return (
    <div className="main-container">
      <div className="nav">
        ...
        <h1>
          <a href={Solution} target="_blank" title="Solution">
            Puzzle #5
          </a>
        </h1>
      ...
      <div
        className="winning-container"
        style={{ backgroundImage: `url(${WinningImage})` }}
      >
        <div></div>
        <h1>You Won!</h1>
        <NavLink
